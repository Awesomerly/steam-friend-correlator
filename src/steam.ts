...
function parseSteamId(id: string) {
  let createdId = new SteamID(id);
  if (createdId.isValidIndividual() === false) {
    throw new ValidationError(400, "Invalid Steam ID");
  }
...
export async function getFriends(id: string) {
  ...
  if (isEmpty(resp)) {
    throw new ValidationError(400, "User does not have public friends");
  }
...
export async function getUserSummaries(idArr: Array<string>) {
  if (idArr.length > 100) {
    throw new ValidationError(400, "NO!!! BECAUSE I SAID SO!!!");
  }
